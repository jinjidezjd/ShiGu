...
{
    // std::cout << initialized_ << " " << imu_buffer_.size() << std::endl;
    // 处理imu数据
    //如果滤波器没有被初始化
    if (!initialized_)
    {
...
{
    //预测环节,参照<<quaternion kinematics for eskf>>P60
    // 时间差
    ...
    // p v R
    Eigen::Vector3d acc_unbias = 0.5 * (last_imu_ptr->acc + cur_imu_ptr->acc) - last_state.acc_bias;    // acc_unbias=am-ab 加速度测量值-偏置
    Eigen::Vector3d gyr_unbias = 0.5 * (last_imu_ptr->gyro + cur_imu_ptr->gyro) - last_state.gyro_bias; // gyr_unbias=wm-wb 角速度测量值-偏置
    Eigen::Vector3d acc_nominal = last_state.R_G_I * acc_unbias + Eigen::Vector3d(0., 0., -g);          // R(am-ab)+g
    state_ptr_->p_G_I = last_state.p_G_I + last_state.v_G_I * dt + 0.5 * acc_nominal * dt_2;            //更新p
    state_ptr_->v_G_I = last_state.v_G_I + acc_nominal * dt;                                            //更新v

    Eigen::Vector3d delta_angle_axis = gyr_unbias * dt; //(wm-wb)*dt
    ...
    if (delta_angle_axis.norm() > DBL_EPSILON)
    {
        ...
        state_ptr_->R_G_I = last_state.R_G_I * dR; //更新转角
    }
...
void ESKF::update(GNSSDataPtr gnss_data_ptr)
{
    ...
    Eigen::Vector3d p_G_GNSS = gnss_data_ptr->point; // enu下的gps坐标
    Eigen::Vector3d &p_G_I = state_ptr_->p_G_I;
    Eigen::Matrix3d &R_G_I = state_ptr_->R_G_I;

    Eigen::Vector3d residual = p_G_GNSS - (p_G_I + R_G_I * p_I_GNSS_); //残差(GPS实际观测坐标-GPS预测坐标)
    //计算H矩阵
...
bool ESKF::process_GNSS_Data(GNSSDataPtr gnss_data_ptr)
{
    ...
    if (!initialized_)
    {
        ...
        if (!initialize())
            return false;
        init_point_ = gnss_data_ptr->point;
        initialized_ = true;
    }

    update(gnss_data_ptr); //更新环节
    return true;
}
...
bool ESKF::initialize(void)
{
    ...
    {
        sum_acc += imu_data->acc; //计算加速度总和
    }
    const Eigen::Vector3d mean_acc = sum_acc / (double)imu_buffer_.size(); //计算平均加速度
    // std::cout<<"[ ESKF ] Mean acc: "<<mean_acc[0]<<" "<<mean_acc[1]<<" "<<mean_acc[2]<<std::endl;

    Eigen::Vector3d sum_err2(0., 0., 0.);
    for (auto imu_data : imu_buffer_)
        sum_err2 += (imu_data->acc - mean_acc).cwiseAbs2();                              //计算误差的平方和
    ...
    state_ptr_->R_G_I = R_I_G.transpose();            //初始转角
    state_ptr_->timestamp = last_imu_ptr_->timestamp; //初始时间戳
    // state_ptr_->imu_data_ptr = last_imu_ptr_;
    state_ptr_->p_G_I.setZero();     //初始位置
    state_ptr_->v_G_I.setZero();     //初始速度
    state_ptr_->acc_bias.setZero();  //初始加速度偏置
    state_ptr_->gyro_bias.setZero(); //初始角速度偏置
    state_ptr_->cov.setZero();       //初始协方差
