...
#include "ekf_eskf_vo/filter_utilities.h"
...
namespace RobotLocalization
{
  ...
  RosFilter<T>::RosFilter(ros::NodeHandle nh,
                          ...
                          std::vector<double> args) : disabledAtStartup_(false),
                                                      enabled_(false),
  ...
  void RosFilter<T>::initialize()
  {
    ...
    // Publisher
    positionPub_ = nh_.advertise<nav_msgs::Odometry>("/ekf_odom", 20);
    pathPub_ = nh_.advertise<nav_msgs::Path>("/ekf_path", 20);
  ...
  template <typename T>
  bool RosFilter<T>::toggleFilterProcessingCallback(ekf_eskf_vo::ToggleFilterProcessing::Request &req,
                                                    ekf_eskf_vo::ToggleFilterProcessing::Response &resp)
  {
  ...
  bool RosFilter<T>::getFilteredOdometryMessage(nav_msgs::Odometry &message)
  {
    ...
    if (filter_.getInitializedStatus())
    {
      ...
      //四元数归一化
      tf::Quaternion q;
      tf::quaternionMsgToTF(message.pose.pose.orientation, q);
      q.normalize();
  ...
  void RosFilter<T>::periodicUpdate(const ros::TimerEvent &event)
  {
    ...
    if (getFilteredOdometryMessage(filteredPosition))
    {
      ...
      {
        // 判断速度和角速度是否超限
        // if (abs(filteredPosition.twist.twist.linear.x) >= 30 || abs(filteredPosition.twist.twist.linear.y) >= 30 || abs(filteredPosition.twist.twist.linear.z) >= 20 || abs(filteredPosition.twist.twist.angular.x) >= 5 || abs(filteredPosition.twist.twist.angular.y) >= 5 || abs(filteredPosition.twist.twist.linear.z) >= 5)
        // {
        //   reset();
        //   return;
        // }
        ...
        pose_stamped.header = filteredPosition.header;
        pose_stamped.pose = filteredPosition.pose.pose;
        nav_path_.header = pose_stamped.header;
        nav_path_.poses.push_back(pose_stamped);
        pathPub_.publish(nav_path_);
      }
  ...
  template <typename T>
  bool RosFilter<T>::setPoseSrvCallback(ekf_eskf_vo::SetPose::Request &request,
                                        ekf_eskf_vo::SetPose::Response &)
  {
